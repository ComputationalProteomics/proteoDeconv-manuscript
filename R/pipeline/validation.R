...
sim_validation_obj <- tar_map(
    ...
    tar_target(
        ...
        preprocess_protein_data(
            ...
            unlog = unlog,
            tpm = tpm,
            remove_samples_pattern = if (str_detect(dataset_id, "dia")) "PBMC|Mix|NK" else "Erythrocyte|Thrombocyte|Library"
        )
    ),
    tar_target(
        ...
        preprocess_protein_data(
            ...
            unlog = unlog,
            tpm = tpm,
            remove_samples_pattern = if (str_detect(dataset_id, "dia")) "NK" else "Erythrocyte|Thrombocyte|Library"
        )
    ),
    tar_target(
        sim_val_simulation,
        simulate_data(
            ...
            cell_types = map_cell_groups(colnames(sim_val_preprocessed_for_simulation |> dplyr::select(-Genes))),
            seed = 4,
            scenario = "even",
